...
class GraphScheduler:
    ...
    def predecessors_satisfied(self, node_id: str, completed_task_node_ids: set[str]) -> bool:
    ...
    def ready_task_nodes(self, node_statuses: dict[str, NodeStatus]) -> list[GraphNode]:
        completed = {
            node_id for node_id, status in node_statuses.items() if status == NodeStatus.COMPLETED
        }
    ...
    def all_tasks_completed(self, node_statuses: dict[str, NodeStatus]) -> bool:
        return all(
            node_statuses.get(node.id) == NodeStatus.COMPLETED for node in self._graph.task_nodes
        )
